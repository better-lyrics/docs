...
export default function Timeline({ ttml }: TimelineProps) {
  ...
  const handleTimelineClick = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      if (!timelineRef.current || totalDuration === 0 || isDragging || didDragRef.current) return;
  ...
  const handleWheel = useCallback(
    (e: React.WheelEvent<HTMLDivElement>) => {
      ...
      const newPanOffset = Math.max(
        0,
        Math.min(totalDuration - newVisibleDuration, mouseTime - mousePercentage * newVisibleDuration)
      );
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      const rect = timelineRef.current.getBoundingClientRect();
      const deltaTime = ((e.clientX - dragStartRef.current.x) / rect.width) * visibleDuration;
      const newOffset = Math.max(0, Math.min(maxPanOffset, dragStartRef.current.offset - deltaTime));
  ...
  const handleZoomIn = useCallback(() => {
    ...
    const newPanOffset = Math.max(0, Math.min(totalDuration - newVisibleDuration, centerTime - newVisibleDuration / 2));
    setZoom(newZoom);
    setPanOffset(newPanOffset);
  }, [zoom, panOffset, visibleDuration, totalDuration]);
  ...
  const handleLineClick = useCallback((index: number, line: ParsedLine, shouldScroll = true) => {
    ...
    <div className="timeline-container">
        ...
        <div className="timeline-controls">
          ...
          {expanded && (
            <div className="zoom-buttons">
              <button className="zoom-btn" onClick={handleZoomOut} disabled={zoom <= 1} title="Zoom out">
                −
              </button>
              <button className="zoom-btn" onClick={handleResetZoom} disabled={zoom === 1} title="Reset zoom">
                ⟲
              </button>
              <button className="zoom-btn" onClick={handleZoomIn} disabled={zoom >= 20} title="Zoom in">
                +
              </button>
            </div>
          )}
      ...
      {expanded && (
        <div className="timeline-content">
          <div
            className={`timeline-track ${zoom > 1 ? 'zoomable' : ''} ${isDragging ? 'dragging' : ''}`}
            ref={timelineRef}
            ...
            <div className="timeline-bars">
              {lines.map((line, index) => {
                ...
                return (
                  <div
                    key={index}
                    className={`timeline-bar ${selectedLine === index ? 'selected' : ''}`}
                    style={{
                      left: `${leftPos}%`,
          ...
          <div className="lines-list">
            {lines.map((line, index) => (
              ...
              >
                ...
                <div className="line-text">{line.text.trim() || '[instrumental]'}</div>
      ...
      <style>{`
        ...
        .timeline-bar {
          ...
          height: 24px;
          background-color: rgba(242, 12, 50, 0.08);
          border: 1px solid rgba(242, 12, 50, 0.4);
          border-radius: 2px;
          cursor: pointer;
          min-width: 2px;
        }
        ...
        .timeline-bar:hover {
          background-color: rgba(242, 12, 50, 0.25);
